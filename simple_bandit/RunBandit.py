...
for obs_int in range(n_intervals-1):  ### For online data, this would have to have a time check
    ...
    # delete all files that would be generated by the new analysis
    ...
    # os.system("find "+outdir+" -type d -name '*bestfit_params.json' -exec sh -c 'f=\"{}\"; mv \"$f\" \"${f%bestfit_params.json}bestfit_params_"+str(obs_int)+".json\"' \;")
    os.system("find "+outdir+" -type d -name '*bestfit_params.json' -prune -exec rm -rf {} +")
    time.sleep(1)
