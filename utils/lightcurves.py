...
def generate_lightcurve(
    ...
    **kwargs,
    ...
    """
    assert os.path.exists(injection_path), "injection file {} does not exist".format(
        injection_path
    )
    if outDir is None:
    ...
    lightcurve_path = os.path.join(
        outDir, "lc_" + model + "_" + lightcurve_label + ".json"
    )
    ...
    args = Namespace(  ## based on Michael's changes to nmma unit test
        ...
        ztf_uncertainties=ztf_uncertainties,
        ztf_ToO=ztf_ToO,
        rubin_ToO=False,
        rubin_ToO_type=None,
    ...
    assert os.path.exists(lightcurve_path), "lightcurve file {} was not created".format(
        lightcurve_path
    )
...
def lightcurve_conversion(lightcurve_path):
    ...
    """
    assert os.path.exists(lightcurve_path), "lightcurve file {} does not exist".format(
        lightcurve_path
    )
    ...
    filters = lines[0].split()[
        2:
    ]  ## skip first 2 items in list ("#", "t[days]"), then get filters
    times = np.array(
        [float(line.split()[0]) for line in lines[1:]]
    )  ## skip first item in list ("#"), then get times
    mags = {
        filt: [float(line.split()[col]) for line in lines[1:]]
        for col, filt in enumerate(filters, start=1)
    }  ## dictionary of magnitudes for each filter

    isot_times = Time(
        times + 40587, format="mjd"
    ).isot  ## sets times to be in isot format, with 40587 being the MJD of 1970-01-01T00:00:00
    ...
    converted_lightcurve_path = os.path.join(
        lightcurve_directory, "converted_" + os.path.basename(lightcurve_path)
    )
...
def read_lightcurve(lightcurve_json, start_time=0.1, cutoff_time=None):
    ...
    """
    lightcurve_df_raw = pd.read_json(
        lightcurve_json
    )  ## this will be in a somewhat messy format
    filters = lightcurve_df_raw.keys()  ## get filters

    filter_dfs = (
        []
    )  ## initialize empty list of dataframes. Each corresponds to a filter
    for filter in filters:
        filt_df = pd.DataFrame(
            lightcurve_df_raw[filter].tolist(),
            columns=["sample_times", filter, f"{filter}_err"],
        )  ## convert each filter to a dataframe
        filter_dfs.append(filt_df)  ## append to list of dataframes
    lightcurve_df = reduce(
        lambda left, right: pd.merge(left, right, on=["sample_times"], how="outer"),
        filter_dfs,
    ).fillna(
        np.inf
    )  ## merge dataframes on sample_times, fill NaNs with np.inf
    lightcurve_df["sample_times"] = round(
        lightcurve_df["sample_times"]
        - lightcurve_df["sample_times"].min()
        + start_time,
        2,
    )  ## subtract start time from sample_times, also rounds to second decimal place for consistency
...
def validate_lightcurve(
    lightcurve_path, min_detections=3, min_time=3.1, all_bands=False, **kwargs
):
    """
    ...
    end_time = (
        start_time + min_time
    )  ## end time is the interval from the start time we want to check for detections

    if all_bands:  ## check if all bands meet requirements
        all_bands_meet_criteria = (
        ...
        for band in lightcurve_df.columns[2:]:
            ...
            num_detections = min_time_interval[band].apply(
                lambda val: 1 if val != np.inf and not np.isnan(val) else 0
            )
            meets_min_detections = num_detections.sum() >= min_detections
            if (
                not meets_min_detections
            ):  ## failure condition, don't need to check other bands
