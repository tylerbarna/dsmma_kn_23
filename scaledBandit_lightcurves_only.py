...
parser.add_argument(
    "--outdir", type=str, default="./fits", help="path to output directory"
)

parser.add_argument(
    "--timeout", type=float, default=1, help="timeout in hours (default: 1 hours)"
)
...
parser.add_argument(
    ...
    type=str,
    default="expanse",
    help="cluster to run on (default: expanse)",
    choices=["msi", "expanse", ""],
)
...
sample_outdirs = [
    os.path.join(outdir, f"{str(idx).zfill(3)}") for idx in sample_idx_offset
]
...
sample_outdirs = [
    sample_outdirs[i] for i in range(len(sample_outdirs)) if i not in idx_to_delete
]
sample_idx_offset = [
    sample_idx_offset[i]
    for i in range(len(sample_idx_offset))
...
def generate_lightcurves(idx_offset, sample_outdir, model=models):
    if type(model) == str:
        model = [model]
    gen_command_array = [
        "python3",
...
with ThreadPoolExecutor() as executor:
    for m in models:
        executor.map(generate_lightcurves, sample_idx_offset, sample_outdirs, repeat(m))
...
for (
    ...
    lightcurve_paths
):  ## skipping the threading for now, should hopefully not be too slow
